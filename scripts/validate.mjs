...
for (const marker of ["const RD03_LINE_COLOR = '#E07B39'"]) {
  if (!html.includes(marker)) throw new Error(`MOD 1 dataset color marker is missing: ${marker}`);
}
...
for (const marker of ['DATASET_LINE_COLORS', 'MAXI_CATEGORY_META', "network: Object.freeze({ label: 'Network', color: '#1E5BA8' })", "'ready-access': Object.freeze({ label: 'Ready Access', color: '#0E9F6E' })", "customer: Object.freeze({ label: 'Customer', color: '#7A3DB8' })", 'applyDatasetLineColors(', 'maxiCategoryColor(line, colorByKey.get(key))', 'datasetColorLegend', 'sourceColor: line.sourceColor', "dashArray: isRemove ? '2,7'"]) {
...
if (!html.includes('if (button) button.textContent = `กำลังสร้าง ${format}…`;')) {
  throw new Error('Export progress must tolerate a missing button element');
}
